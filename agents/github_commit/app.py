"""GitHub test-catalog commit agent entrypoint."""
...
class GitHubCommitAgentLogic:
    ...
    async def _handle_commit_task(self, task: Dict[str, Any]):
        ...
        try:
            ...
            repo = await asyncio.to_thread(self.github_client.get_repo, f"{repo_owner}/{repo_name}")
    ...
    async def start(self) -> bool:
        """Start exactly one background consumer when GitHub is configured."""
        if not self.github_client:
            logger.warning("GitHub commit consumption is disabled until GITHUB_PAT is configured.")
            return False
        if self.consumer_task and not self.consumer_task.done():
            return True
        self.consumer_task = asyncio.create_task(self.start_consumer_loop())
        return True
...
class GitHubCommitAgentExecutor(AgentExecutor):
    ...
    async def cancel(
        ...
        logger.info('GitHub commit consumer stopped.')
...
if __name__ == '__main__':
    ...
    AGENT_PORT = int(os.environ.get("AGENT_PORT", 8011))
    AGENT_ID = os.environ.get("AGENT_ID", "GitHubCommitAgent")
    ...
    uvicorn.run(PrometheusMiddleware(cors_app, "github-commit"), host='0.0.0.0', port=AGENT_PORT)
