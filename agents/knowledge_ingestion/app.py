"""Knowledge and ontology ingestion agent entrypoint."""
...
from utils.agent_ontology import load_ontology, ontology_records
...
class ArtifactManager:
    """Simplified, internal client for RustFS and PostgreSQL metadata management."""
    def __init__(self):
    ...
    async def register_new_artifact_version(self, data_content: str, artifact_type: str) -> str:
        """Uploads metadata to RustFS and registers the new version in PostgreSQL."""
        ...
        # 2. Upload metadata to RustFS (using asyncio.to_thread for blocking I/O)
        data_bytes = data_content.encode('utf-8')
        
        def blocking_upload():
            self.object_store.ensure_bucket()
            self.object_store.write_bytes(object_name, data_bytes, "application/json")
...
class LLMFineTuningAgentLogic:
    ...
    def _replace_source_records(self, source: str, records: List[Dict[str, str]]) -> int:
        """Replace one evidence source without deleting other RAG or ontology records."""
        ...
        points = [
            models.PointStruct(
                id=str(uuid.uuid5(uuid.NAMESPACE_URL, f"aqe:{source}:{record['id']}")),
                vector=embed_text(record["text"]),
                payload={
                    "text_chunk": record["text"],
                    "source": source,
                    "record_id": record["id"],
                    "record_kind": record.get("kind", source),
                },
            )
    ...
    async def ingest_and_register_knowledge(self, product_specs: str) -> Dict[str, str]:
        ...
        def blocking_qdrant_ops():
            records = [{"id": str(index), "text": chunk} for index, chunk in enumerate(chunks)]
            return self._replace_source_records("product_knowledge", records)
    ...
    async def ingest_agent_ontology(self) -> Dict[str, str]:
        ...
        try:
            ...
            count = await asyncio.to_thread(
                self._replace_source_records, "agent_ontology", records
            )
            version_id = await self.artifact_manager.register_new_artifact_version(
                data_content=json.dumps(ontology, sort_keys=True),
                artifact_type="AGENT_ONTOLOGY",
            )
            return {
                "status": "SUCCESS",
                "message": f"Ingested {count} versioned agent ontology records.",
...
async def ingest_ontology_handler(request: Request):
    """Ingest the repository-versioned ontology; request bodies cannot replace it."""
    result = await AGENT_LOGIC.ingest_agent_ontology()
...
if __name__ == '__main__':
    ...
    uvicorn.run(PrometheusMiddleware(main_app, "knowledge-ingestion"), host='0.0.0.0', port=AGENT_PORT)
