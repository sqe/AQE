"""Change-detection orchestration agent entrypoint."""
...
class ChangeDetectionAgentLogic:
    ...
    def __init__(self, target_url: str, product_specs: str, repo_owner: str, repo_name: str):
        ...
        self.capture_client = A2AClient(
            self.http_client,
            url=os.getenv("STATE_CAPTURE_URL", "http://webpage_state_capture_agent:8002")
        )

        self.generation_client = A2AClient(
            self.http_client,
            url=os.getenv("TEST_GENERATION_URL", "http://test_generation_agent:8001"))
        
        self.execution_client = A2AClient(
            self.http_client,
            url=os.getenv("AGENT_TEST_EXECUTION_URL", "http://test_execution_agent:8003"))
...
if __name__ == '__main__':
    ...
    uvicorn.run(PrometheusMiddleware(cors_app, "change-detection"), host='0.0.0.0', port=AGENT_PORT)
